...
void BGI_server(DWORD param)
{
  ...
  if(options & MODE_FULLSCREEN)
  {
	...
	window.wnd = CreateWindow(WINDOW_CLASS_NAME, "Pong", WS_VISIBLE | WS_POPUP, 0, 0, window.width, window.height, NULL, NULL, BGI_getInstance(), NULL);
  }
  else 
  {
    //window.wnd = createWindow(WINDOW_CLASS_NAME, "Graphics");
	  window.wnd = createWindow(WINDOW_CLASS_NAME, "Pong");
  }
